...
def draw_results_ssd(detected,input_img,faces,ad,img_size,img_w,img_h,model,time_detection,time_network,time_plot):
    ...
    #        cv2.rectangle(input_img, (x1,y1), (x1+w,y1+h), [0,255,0], 2)
            ...
            #input_img1=input_img
            # draw(input_img1,rotate_degree)
            input_img[yw1:yw2 + 1, xw1:xw2 + 1, :] = img
            input_img1[yw1:yw2 + 1, xw1:xw2 + 1, :] = img
        #cv2.imshow("result", input_img1)
    
        return input_img,rotate_degree #,time_network,time_plot

def main(file):
    try:
        os.mkdir(file[3].replace('\n',''))
    except OSError:
        pass
    try:
        os.mkdir(file[3].replace('\n','')+"/"+file[4].replace('\n',''))
    ...
    weight_file1 = 'pre-trained/fsanet_capsule.h5'
    model1.load_weights(weight_file1)
    print('Finished loading model 1.')

    weight_file2 = 'pre-trained/fsanet_var_capsule.h5'
    model2.load_weights(weight_file2)
    print('Finished loading model 2.')

    weight_file3 = 'pre-trained/fsanet_noS_capsule.h5'
...
# capture video
    ...
    #input_img=cv2.rotate(input_img,cv2.ROTATE_90_CLOCKWISE)
    input_img=imutils.resize(input_img,height=1000)
    input_img1 = input_img
    ...
    while True:
        ...
        if len(rotate_degree)>1:
            ...
            str5=file[3].replace('\n','')+"/"+file[8].replace('\n','')
            #file2.write(tet)
            if pitch > 3.2 and yaw >-15 and yaw <15:
                cv2.imwrite(str4+"/{}.jpg".format(m),input_img)
                m=m+1
                top=top+1
                # if yaw <-8 or yaw >8:
                #     pitch_top=pitch_top+1
            if pitch < -14 and yaw >-15 and yaw <15:
                cv2.imwrite(str1+"/{}.jpg".format(m),input_img)
                m=m+1
                bottom=bottom+1
                # if yaw <-10 or yaw >10:
                #     pitch_bottom=pitch_bottom+1
            if yaw > 15 and pitch > -5:
                cv2.imwrite(str2+"/{}.jpg".format(m),input_img)
                m=m+1
                # right=right+1
            if yaw < -15 and pitch > -5:
                cv2.imwrite(str3+"/{}.jpg".format(m),input_img)
                m=m+1
                # left=left+1
            if yaw>-12 and yaw<12 and pitch >-12 and pitch <2:
                cv2.imwrite(str5+"/{}.jpg".format(m),input_img)
                m=m+1
                pro=pro+1
            # if yaw<-10 or yaw>10 or pitch <-10 or pitch >2:
            #     pro_sai=pro_sai+1
            #re="top: "+"{}".format(pitch_top)+"/"+"{}".format(top)+"  bottom: "+"{}".format(pitch_bottom)+"/"+"{}".format(bottom)+"  pro:  "+"{}".format(pro_sai)+"/"+"{}".format(pro)+"\n"
            #file2.write(re)
